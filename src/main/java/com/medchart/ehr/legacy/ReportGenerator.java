...
public class ReportGenerator {
    ...
    public String generateEncounterSummary(LocalDateTime startDate, LocalDateTime endDate) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(filePath))) {
            ...
            for (Object[] row : results) {
                ...
                writer.println("  Provider: " + (row.length > 9 ? row[9] : "") + " " + (row.length > 10 ? row[10] : ""));
                writer.println();
            }
        } catch (IOException e) {
    ...
    /**
     * Securely delete temporary files containing PHI.
     * HIPAA requires that PHI in temporary storage is cleaned up promptly.
     */
    private void cleanupTempFile(String filePath) {
        ...
        try {
            ...
            if (Files.exists(path)) {
                Files.delete(path);
                log.debug("Cleaned up temp PHI file: {}", filePath);
            }
        } catch (IOException e) {
            log.warn("Failed to clean up temp PHI file: {}. Manual cleanup required.", filePath, e);
        }
    }
}
