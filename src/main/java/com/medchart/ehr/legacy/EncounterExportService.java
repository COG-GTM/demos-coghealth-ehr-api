...
public class EncounterExportService {
    ...
    @AuditAccess(action = AuditAction.EXPORT, resourceType = "Encounter", description = "Bulk encounter export")
    public byte[] exportEncountersForDateRange(LocalDate startDate, LocalDate endDate, int page, int size) {
        ...
        log.info("Exported {} encounters for date range {} to {} (page {}, size {})",
            results.size(), startDate, endDate, page, size);
        return csv.toString().getBytes();
    }

    @PreAuthorize("hasRole('PHI_EXPORT')")
    @AuditAccess(action = AuditAction.EXPORT, resourceType = "Encounter", description = "Patient encounter history export")
    public byte[] exportPatientEncounterHistory(Long patientId, int page, int size) {
        ExportLimits.validatePagination(page, size);
        ...
        export.append("SSN: ").append(ExportLimits.maskSsn(patientData[3])).append("\n");
    ...
    @PreAuthorize("hasRole('PHI_EXPORT')")
    @AuditAccess(action = AuditAction.EXPORT, resourceType = "Patient", description = "Patient file export")
    public void exportAllPatientsToFile(String filePath, int page, int size) {
        ExportLimits.validatePagination(page, size);

        Query query = entityManager.createNativeQuery(
            "SELECT id, mrn, first_name, last_name, date_of_birth, " +
            "email, phone_home, phone_mobile, street1, city, state, zip_code " +
            "FROM patients WHERE active = true ORDER BY id");
        query.setFirstResult(ExportLimits.offset(page, size));
        query.setMaxResults(size);
