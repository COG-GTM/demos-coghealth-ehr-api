...
public class EncounterExportService {
    ...
    public byte[] exportPatientEncounterHistory(Long patientId) {
        ...
        List<Object[]> patientRows = patientQuery.getResultList();
        if (patientRows.isEmpty()) {
            throw new EntityNotFoundException("Patient not found with id: " + patientId);
        }
        Object[] patientData = patientRows.get(0);
        
        Query encounterQuery = entityManager.createNativeQuery(
            "SELECT encounter_number, encounter_date_time, encounter_type, status " +
            "FROM encounters WHERE patient_id = ?1 ORDER BY encounter_date_time DESC");
        ...
        for (Object[] enc : encounters) {
            export.append("Encounter #: ").append(enc[0]).append("\n");
            export.append("Date: ").append(enc[1]).append("\n");
            export.append("Type: ").append(enc[2]).append("\n");
            export.append("Status: ").append(enc[3]).append("\n");
