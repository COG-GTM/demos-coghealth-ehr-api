...
public class LegacyPatientLookup {
    ...
    @AuditAccess(action = AuditAction.READ, resourceType = "Patient", description = "Legacy lookup by MRN")
    public Patient findPatientByMrn(String mrn) {
        try {
            ...
            log.warn("Patient not found for MRN lookup");
            return null;
        }
    }

    @AuditAccess(action = AuditAction.SEARCH, resourceType = "Patient", description = "Legacy search by last name")
